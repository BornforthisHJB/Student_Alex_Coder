...
class scr1(object):
    ...
    def parse(self):
        html = scr1().requests("https://static1.scrape.center/")
    ...
    def scrape_index(self, page=None):
        index_url = f"{BASE_URL}/page/{page}"
        return scr1().requests(index_url)
...
def main(self):
    run_result = scr1().parse()
    for page in range(1, TOTAL_PAGE + 1):
        index_html = scr1().scrape_index(page)
        detail_urls = scr1().parse_index()
        # print(detail_urls)
        for url in detail_urls:
            html = scrape_details(url)
            r = parse_details(html)
        return index_html
    return run_result
